...
_Last updated: 2026-07-19. Work lives on the `desktop-os` branch (2 commits ahead of `main`). Nothing is pushed or deployed yet._
...
- **13/13 jest tests pass** (4 suites), `npm run build` compiles clean (~73KB gzipped JS).
...
- Deleted as part of this work: `background.png` (4.1MB, replaced by CSS gradient), `leaf1-3.png` + `leaf.js` (leaf animation retired), root `ANDY_HUANG_LING_RESUME_2024.pdf`, `src/logo.svg`. `public/resume.pdf` is the served resume; `chatroom.mp4` stays (Projects demo).
- EmailJS keys in `sendemail.js` are public by design (client-side EmailJS); guestbook is localStorage per-browser (UI says so honestly).
...
4. Post-MVP parked ideas: Minesweeper in Games (~1-1.5 days), dad-joke app (`joke.js` is ready, ~1-2 hrs), phone task switcher, WebP desktop wallpaper.
